...
function App() {
  ...
  // Auto-load cached ML model on startup
  useEffect(() => {
    useAppStore.getState().checkAndAutoLoadMlModel()
  }, [])

  // Auto-reanalysis after ML model finishes loading
  useEffect(() => {
    if (mlPendingReanalysis && input.trim()) {
      useAppStore.setState({ mlPendingReanalysis: false })
      analyzeText(input)
    }
  }, [mlPendingReanalysis, input, analyzeText])
  ...
  if (fullscreenView) {
    return (
      <FullscreenViewer
        ...
        changedLinesSet={changedLinesSet}
        onDownload={handleDownload}
        onDownloadZip={handleDownloadZip}
        onDownloadGzip={handleDownloadGzip}
      />
    )
  }
        ...
        <div className="flex flex-col flex-1 min-h-0 lg:overflow-hidden">
          <div className="flex flex-col min-h-0 lg:overflow-hidden flex-1">
            ...
            <div className="flex-shrink-0">
              ...
              {analysisCompleted && mlLoadingState !== 'ready' && !mlExplicitlyConfigured && !dismissedBanners.has('ml') && (
                <DetectionBanner
                  icon={
              ...
              {mlNameDetectionEnabled && mlLoadingState !== 'ready' && analysisCompleted && mlModelCached === false && !dismissedBanners.has('mlRedownload') && (
                <DetectionBanner
                  icon={
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              ...
              <Editor
                ...
                onToggleRulesets={() => showSuggestions ? dismissSuggestions() : setShowSuggestions(true)}
                showSettings={showSettings}
                settingsPanel={<SettingsPanel />}
                onCloseSettings={dismissSettings}
                onToggleSettings={() => showSettings ? dismissSettings() : setShowSettings(true)}
              />
            </div>
          </div>
        </div>
